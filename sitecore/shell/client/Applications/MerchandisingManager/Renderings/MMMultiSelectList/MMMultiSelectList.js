define(["sitecore", "knockout", "underscore"], function (sc, ko, _) {
    ...
    sc.Factories.createBehavior("MMMultiSelectList", {
        events: {
            "change .sc-cb": "check",
            "click .cb": "checkCell",
        ...
        afterRender: function () {
            var viewMode = this.model.get("view"),
              headerCheckboxContainer,
              rowCheckboxContainer;

            switch (viewMode) {
                case "TileList":
                    headerCheckboxContainer = $(); // TileList currently doesn't have support for a check-all header.
                    rowCheckboxContainer = this.$el.find(".sc-tile-checkBoxContainer");
                    break;
                case "IconList": // IconList currently does not support MultiSelectList behavior
                    return;
                case "DetailList":
                default:
                    headerCheckboxContainer = this.$el.find("thead tr");
                    rowCheckboxContainer = this.$el.find(".sc-table tbody tr");
                    break;
            }
            ...
            if (items && items.length) {
                _.each(headerCheckboxContainer, this.insertGlobalcheck, this);
                _.each(rowCheckboxContainer, this.insertcheck, this);
                this.globalCheck = this.$el.find(".sc-cball");
                this.on("addrow", this.addrow);

                this.$el.find(".sc-tile-checkBoxArea").off("click").on("click", function (evt) {
                    evt.stopPropagation();
                    this.checkCell(evt);
                }.bind(this));
            }
        ...
        insertcheck: function (el) {
            var $el = $(el),
              viewMode = this.model.get("view"),
              checkbox,
              emptyCheck;

            switch (viewMode) {
                case "TileList":
                    checkbox = "<input class='sc-cb' type='checkbox' onclick='event.stopPropagation();' />";
                    emptyCheck = "";
                    break;
                case "IconList": // IconList currently does not support MultiSelectList behavior
                    return;
                case "DetailList":
                default:
                    checkbox = "<td class='cb'><input class='sc-cb' type='checkbox' onclick='event.stopPropagation();' /></td>";
                    emptyCheck = "<td></td>";
                    break;
            }
        ...
        check: function (evt) {
            ...
            switch (viewMode) {
                case "TileList":
                    headerCheckboxContainer = $(); // TileList currently doesn't have support for a check-all header.
                    $rows = this.$el.find("sc-tileList-item-container");
                    $row = $current.closest(".sc-tileList-item-container");
                    break;
                case "IconList": // IconList currently does not support MultiSelectList behavior
                    break;
                case "DetailList":
                default:
                    ...
                    $rows = this.$el.find(".sc-table tbody tr");
                    break;
            }
