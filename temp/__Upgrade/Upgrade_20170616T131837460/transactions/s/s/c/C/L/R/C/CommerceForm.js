...
(function (speak) {
    ...
    speak.component(["jquery", "bootstrap"], function () {
        ...
        var intializeBinding = function () {
            ...
            this.FormData.on("change", function (val, eventInfo) {
                if (!this.BindingTarget) {
                    return;
                }
                var propertyName = eventInfo.eventName.split(":")[1];
                this.BindingTarget[propertyName] = val;
            }, this);
        },
        ...
        return {
            ...
            getCommerceFormData: function () {
                var formData = this.getFormData();
                var cleanedFormData = {};

                $.each(formData, function (key, value) {
                    // Remove the key-prefix 'CF_'
                    var originalKey = key.substring(3).replace("-", ".");
                    cleanedFormData[originalKey] = value;
                });

                return cleanedFormData;
            },

            getCommerceCustomerFormData: function () {
                var formData = this.getFormData();
                var cleanedFormData = {};

                $.each(formData, function (key, value) {
                    // Remove the key-prefix 'CF_' but keep the dashes
                    var originalKey = key.substring(3);
                    cleanedFormData[originalKey] = value;
                });
